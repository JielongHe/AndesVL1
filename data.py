...
describles_answ['swap_manipulation'] = "E. Both face swap and text swap."  # [0, 0, 0, 0, 1, 0]
describles_answ['attribute_manipulation'] = "F. Both face attribute and text swap."  # [0, 0, 0, 0, 0, 1]
...
class OriDGM4Dataset(Dataset):
    ...
    def __init__(self, split, data,image_processor, tokenizer,max_words=30, image_res=224):
        self.name = "DGM4"
        ...
        self.patch_size =14
        self.image_token_id =  tokenizer.vocab['<|vision_pad|>']
    ...
    def __getitem__(self, index):
        ...
        img_dir = os.path.join('/scratch-shared/npu/qwen/SAMM_data', ann['image'])
        #
        # print(ann['image'])
        # img_dir = ann['image']
        ...
        decoded_string = image_dir_all.encode('utf-8').decode('unicode_escape')

        # Replace all non-ASCII characters with an empty string (or any specific replacement)
        image_dir_all = re.sub(r'[^\x00-\x7F]+', '', decoded_string)

        image_dir_all = image_dir_all.replace("'", "")
        image_dir_all = image_dir_all.replace("._", "_")
        image_dir_all = image_dir_all.replace("..", ".")
        image_dir_all = image_dir_all.replace(",_", "_")
        ...
        if any(keyword in label for keyword in ['swap_manipulation', 'attribute_manipulation']):
            try:
                x, y, w, h = self.get_bbox(ann['fake_image_box'])
                has_bbox = True
            except Exception:
        ...
        # pixel_values.append(transform(processed_image))
        ...
        question = describe_temple + caption + describe_ques_latter + face_text_locate
        answer = describles_answ[label]
        ...
        num_image_tokens = image.size[0] * image.size[1] // (base * base)
        ...
        assistant_start_idx = len(self.tokenizer(prompt[:assistant_start], add_special_tokens=False).input_ids)
        ...
        # return pixel_values, questions, answer, fake_text_pos_list, caption, fake_image_box
...
class VQAInstructDataset(BaseDataset):
    def __init__(self, split, max_length=1024):
        ...
        split_actions = {
            ...
            data.train_test_split(test_size=0.05, seed=42)['test'].train_test_split(test_size=0.5, seed=42)['train'],
            'test': lambda data:
            data.train_test_split(test_size=0.05, seed=42)['test'].train_test_split(test_size=0.5, seed=42)['test']
        }
...
class TheCauldronDataset(BaseDataset):
    ...
    def load_all_configs(self, split):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=16) as executor:  # Limit the number of workers
            with tqdm(total=len(cauldron_config_names), desc="Total Progress") as total_pbar:
                futures = {executor.submit(self.load_config, config_name, split): config_name for config_name in
                           cauldron_config_names}
    ...
    def __getitem__(self, idx):
        ...
        image_data = self.images_df.loc[
            (self.images_df['image_index'] == image_idx) & (self.images_df['source'] == source), 'images'].values[0][0][
            'bytes']
